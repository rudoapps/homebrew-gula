...
show_agent_help() {
    ...
    echo ""
    echo -e "  ${BOLD}gula undo${NC}"
    echo "      Lista backups disponibles de archivos modificados"
    echo ""
    echo -e "  ${BOLD}gula undo <número>${NC}"
    ...
    echo "  gula undo                     # Ver backups disponibles"
    echo "  gula undo 1                   # Restaurar primer backup"
    echo ""
    ...
    case "$subcommand" in
        ...
        undo)
            ...
            if [[ "$1" == "list" ]]; then
                ...
                list_file_backups
                echo ""
                echo -e "${BOLD}Para restaurar un backup:${NC} gula undo <número>"
                echo -e "${DIM}Ejemplo: gula undo 1${NC}"
                echo ""
            fi
            ;;
        help|--help|-h)
