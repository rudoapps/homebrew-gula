...
show_help() {
  ...
  echo "  gula create flutter --key=mi_clave --bb-user=mi_user --bb-token=APP_PWD --bb-workspace=rudoapps"
  echo "  gula create python --key=mi_clave --bb-user=mi_user --bb-token=APP_PWD --bb-workspace=rudoapps --bb-repo=mi-api"
  echo "  gula create ios --key=mi_clave --bb-user=mi_user --bb-token=APP_PWD --bb-workspace=rudoapps --bb-public"
  echo ""
...
if [ "$COMMAND" == "install" ]; then
  ...
  if [ -n "$BB_TOKEN" ]; then
    if [ -z "$BB_USER" ] || [ -z "$BB_WORKSPACE" ]; then
      ...
      echo "Uso: gula create <platform> --key=XXX --bb-user=usuario --bb-token=YYY --bb-workspace=mi_workspace [--bb-repo=nombre_repo]"
      exit 1
    fi
